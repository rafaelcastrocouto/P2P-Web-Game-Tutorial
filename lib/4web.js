...
import * as Ipfs from 'https://cdn.jsdelivr.net/npm/ipfs@0.66.0/dist/index.min.js';
// https://github.com/ipfs/js-ipfs/tree/master/docs
// https://js.ipfs.tech
...
/* ====================== OPTS ====================== 
  ...
  opts = {
    ...
    playerLimit: 64, // players
    peerTimeout: 3,  // seconds
    ...
    ipfsOpts: { https://github.com/ipfs/js-ipfs/blob/master/docs/CONFIG.md },
    ipfs: false
  }
...
/* ====================== IPFS ====================== */
  ...
  const connectIpfs = async function () {
    node = await window.Ipfs.create(opts.ipfsOpts);
  };
  ...
  const getFile = async function (cid) {
    log('getting file');
    const stream = node.cat(cid.textContent);
...
/* ====================== PEER CONNECTIONS ====================== */
  ...
  const connectionOpen = function (id, connection) {
    ...
    list[id].interval = setInterval(function () {
      const now = new Date().valueOf();
      if (!(id in list) || (now - list[id].lastDate > opts.peerTimeout * 1000)) {
        eventTarget.dispatchEvent(new CustomEvent('left', {
          detail: { id, connection, connectionDate: list[id]?.connectionDate }
        }));
        clearInterval(list[id]?.interval);
        delete list[id];
      } else connection.send({ net4webPing: true, date: now });
    }, 1000);
  };
